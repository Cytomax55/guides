...
The best method to avoid double signing is to setup a new validator wallet and private key doing a normal installation of Findora via our guide on a new server. Once `fn show` is working with the new wallet info and when the docker container is synced up you can then proceed with the following:

### On your old server
- Stop Docker To Stop Signing
```bash
docker stop findorad && docker rm findorad || exit 1
```
...
- Create files node.mnemonic, replace `mnemonic phrases` with your 24 words from tmp.gen.keypair file:
```bash
...
- Restart Docker Container with previous keys:
```bash
docker run -d \
